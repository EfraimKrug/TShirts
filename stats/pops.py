...
def getPop(country):
    for x in range (0, len(data['pops'])):
        for k in data['pops'][x]:
            if k.replace('-',' ').replace('&',' & ') == country:
                return str(data['pops'][x][k])
                # print(k.replace('-',' ').replace('&',' & ') + " ==>> " + str(data['pops'][x][k]))

    # print (str(x) + " ==>> " + str(data['pops'][x]))
# v = getPop("Switzerland")
